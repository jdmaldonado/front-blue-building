...
export function CardRegisterDialog({
  ...
  const handleSetup = (): void => {
    if (buildingId === '' || selectedDoor?.localId === null || selectedDoor?.localId === undefined) {
      return;
    }
    reader.start({ buildingId, localId: selectedDoor.localId });
  };
  ...
  return (
    <Dialog
      ...
      footer={
        <>
          ...
          <Button onClick={handleSave} loading={create.isPending} disabled={tag.trim() === ''}>
            Crear tarjeta
          </Button>
        </>
      }
    >
      <div className="flex flex-col gap-4">
        ...
        <Button intent="neutral" appearance="outline" onClick={handleSetup} disabled={!canSetup} loading={busy}>
          <Radio size={16} />
